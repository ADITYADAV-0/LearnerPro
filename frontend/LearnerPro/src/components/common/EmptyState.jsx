...
const EmptyState = ({onActionClick, title, discription, buttonText}) => {
  return (
    <div className=' flex flex-col items-center justify-center py-16 px-6 text-center bg-linear-to-br from-slate-50/50 to-white border-2 border-dashed border-slate-200 rounded-3xl'>
        <div className=" inline-flex items-center justify-center h-16 w-16 rounded-2xl bg-linear-to-br from-slate-100 to-slate-200/50 mb-6">
            <FileText className=' w-8 h-8 text-slate-400' strokeWidth={2}/>
        </div>
        <h3 className='text-lg font-semibold text-slate-900 mb-2'>{title}</h3>
        <p className='text-sm text-slate-500 mb-8 max-w-sm leading-relaxed'>{discription}</p>
        {buttonText && onActionClick && (
            ...
            >
                ...
                <div className="absolute inset-0 bg-linear-to-r from-white/0 via-white/20 to-white/0 translate-x-[-100%] group-hover:traslate-x-[100%] transition-transform duration-700"/>
            </button>
        )}
    </div>
  )
