...
const FlashcardManager = ({documentId}) => {
    ...
    const handleGenerateFlashcards = async () => {
      if (!documentId) {
        toast.error('No document selected to generate flashcards from.');
        return;
      }
      ...
      try {
        if (import.meta.env && import.meta.env.DEV) {
          console.log('Generating flashcards for documentId:', documentId);
        }
        const res = await aiService.generateFlashcards(documentId)
        if (import.meta.env && import.meta.env.DEV) {
          console.log('generateFlashcards response:', res);
        }
